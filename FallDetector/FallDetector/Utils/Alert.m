...
+ (void) show : (NSString *) title andMessage : (NSString *) message cancelButtonTitle:(NSString *)cancelTitle otherButtonTitle1:(NSString *)otherTitle andOtherButtonTitle2:(NSString*)otherTitle2 tag:(NSInteger)tag WithDelegate:(id<UIAlertViewDelegate>) delegate{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:message delegate:nil cancelButtonTitle:cancelTitle otherButtonTitles:otherTitle, otherTitle2, nil];
    alert.tag = tag;
    alert.delegate = delegate;
    [alert show];
}
